    ...
    slots: [
        {
            ...
            holdAxes: ['transition'],
            directive:
                'Foreground SPATIAL ORGANIZATION as the primary differentiator for this realization. Choose a distinctive spatial configuration (zone count, zone shape, channels, asymmetric thirds, central corridor, wide channels) and let the setup paragraph make that spatial signature visible. Restart and transition logic should remain consistent with the archetype — differentiation lives in the space, not in the restart rules. Do NOT frame this as the "starter", "introductory", or "simpler" version; it is one of three parallel designs.',
        },
        {
...
const APPLYING_PROFILE: EmphasisVariationProfile = {
    ...
    bandwidthRule:
        'The three activities should share their CORE environmental configuration (same archetypal zone structure, same restart logic, same numerical relationship) and vary MODESTLY along one micro-parameter per activity (working area size, timing window, scoring threshold). Differentiation should feel like repeated exposure to the same problem with intentional small environmental differences — NOT three different environments and NOT a difficulty ramp. A coach reading the three setups side by side should perceive the same problem revisited with a small environmental shift each time.',
    slots: [
        {
        ...
        },
        {
            ...
            directive:
                'Hold transition logic, overload relationship, and scoring system identical to the other two activities. Vary MODESTLY along one spatial micro-parameter — e.g. a slightly different working area size, a slightly compressed central channel, or a slightly shifted zone boundary. The activity should feel like the same problem revisited with a small spatial shift, NOT a different environment. Do NOT frame this as "dialled-up pressure" or "intermediate"; it is one of three repeated exposures.',
        },
        {
