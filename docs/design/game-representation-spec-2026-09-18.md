# Game Representation Specification — revision 20, with Christian's ownership decisions

> **Revision 18 (25 September) — PHASE B CLUSTER 2 CORRECTED.** Adds SD-83 to SD-85. Only
> support-capable, authoritative contributions establish an element; exclusions, assumptions, engine
> wording and out-of-boundary notes do not. **19 of 53 element classes were falsely instantiated** and
> are removed — including two exclusions that had been satisfied by creating the very thing they forbid.
> A collection the schema fixes at exactly one holds **one** element that several contributions support,
> read from the invariant as data and **not** a relaxation of SD-47. Exclusions route through SD-75's
> negative-existence treatment. The resulting 47-line reduction is **diagnostic correction, not progress
> against knowledge gaps**. Activity generation remains frozen.
>
> **Revision 17 (24 September) — PHASE B CLUSTER 1 CORRECTED.** Adds SD-78 to SD-82. Independently
> authored `REQUIRED_RANGE` narrowings on one structural property are **composed by intersection**: the
> engine determines what they jointly permit rather than choosing among them. The corpus's only collision
> resolves to a single member, `line_crossed`, supported by all three contributions — it was convergence,
> not conflict. A set of permitted alternatives is **never** a resolved value, and regression coverage
> holds that. Phase B now records whether each correction is local, area-reusable, or a general rule.
> The pilot boundary is set as a readiness criterion, not a date. Activity generation remains frozen.
>
...
> Participation State** under Performers (§5.3), the latter bound to knowledge-core EM-0007 rather than
> re-authored. The `BY_CONSTRUCTION` sentinel lets the schema itself entail a requirement, provided the
> invariant is named and mechanically tested. **The eight areas are unchanged; there is no ninth area.**
> Activity generation remains frozen, and the downstream findings are recorded, not repaired.
>
...
| SD-67 | **Satisfied by construction.** *"A structural requirement that is necessarily true by the authoritative representation schema may be satisfied by construction, provided the exact schema invariant that entails it is named and mechanically testable."* This is **not** `NO_ROW` and not silence: the contract still makes a structural claim, and the schema supplies its entailment. *"Do not generalize 'by construction' beyond mechanically necessary schema consequences."* Registered as the `BY_CONSTRUCTION` sentinel with named invariants; the named invariant is executed at load and an item whose invariant is absent, untestable or false is refused. GF2-01 is satisfied by `SINGLE_RECTANGULAR_PLAYING_AREA` | `contractSentinels.BY_CONSTRUCTION`; `load.ts` | [C27] |
| SD-68 | **A contract may not exceed its source.** GF2-02's source says *"Participants share an adaptive opportunity space."* The contract's stronger claim — that no layout element partitions the teams — *"is one interpretation of that source, not something the source itself entails"*. The item is restated back to what its source supports; the no-team-partition clause is recorded as **unsupported contract interpretation rather than canonical structural knowledge**, and the representative requirement is preserved for future authoring. **No team-partition capability is added from this item** | task register C9 | [C27] |
| SD-69 | **An assumption is not represented structure.** GF2-15 is `ASSUMED` and mixes structure with play, so its typing as a structural Game Representation claim is retired. The assumption is preserved as a **diagnostic/boundary condition outside structural entailment**: *"It may constrain interpretation where our rules permit assumptions to do so, but it cannot establish represented structure"* | task register C10 | [C27] |
| SD-70 | **Action Restriction** — a bounded capability in the Rules area: *"an authored rule that changes whether an otherwise available game action or target is legally eligible."* Sufficient for banned/eligible receivers, forced action or pass order, and prohibited action or direction classes. *"Keep the first implementation narrow. Do not build a general rules ontology."* The purpose is structural: the resolved game must be able to record when a rule removes an otherwise legally available action. **It records authored legal eligibility only** and infers nothing about tactical availability or perceptual affordance | rows `R1`–`R4` | [C27] |
| SD-71 | **Performer Participation State** — a local refinement under Performer holding *"the realized participant state of a performer where authoritative knowledge requires it"*, able to express that a neutral is an active participant throughout rather than rotating, waiting or observing. **Bound to the canonical Participant State concept, not duplicated**: knowledge core EM-0007, dimension EMD-0014, parameters EMP-0040 and EMP-0042. *"Knowledge owns the concept; Game Representation holds its realized structural value."* **No ninth area** | rows `P11`–`P13`; `register.boundVocabularies` | [C27] |
...
| SD-78 | **`REQUIRED_RANGE` composition by intersection.** *"Independently authored, support-capable `REQUIRED_RANGE` contributions that apply to the same structural property may be composed by intersection when they are simultaneously authoritative and in scope. The contributions remain independent. The engine is not choosing among them or establishing precedence; it is determining the jointly permitted value set."* Exactly one member → `RESOLVED:ENTAILED`, **every contributing narrowing retained as support**; more than one → `FREE(choice)` under SD-39; zero → genuine `UNRESOLVED`, contributing narrowings preserved. **Authored ordering may not resolve a multi-member intersection — RC-29 remains unresolved** | `derive.ts`, `classify.ts` | [C29] |
| SD-79 | **The five prose-wrapped sets become machine-readable.** Restatement only: *"Preserve exactly the authored membership of each set. Do not add, remove, interpret, prioritize or otherwise normalize the alternatives beyond what is required to represent the existing authored members structurally. Where an authored order exists, preserve it as provenance/knowledge but do not apply it unless and until its role is separately ruled"* | `stage-b/corpus-restatements.json` | [C29] |
| SD-80 | **A set of permitted alternatives is never the resolved value of a single-valued property.** The two lines reported `RESOLVED:ENTAILED` while holding a set are the **same derivation defect**, not separate knowledge defects. Regression coverage specifically prevents a `REQUIRED_RANGE` set being emitted as the resolved value merely because only one contribution reaches the line | `emit.ts`, `gates.ts`; increment 5 tests | [C29] |
| SD-81 | **Phase B tracks the reach of each correction.** When a cluster is resolved, record whether its correction is **local to that knowledge**, **reusable within a representation area**, or **a general derivation/composition rule**. *"I want to see whether the large failed-line population is collapsing under a relatively small number of general mechanisms rather than treating reduction in the raw failure count as progress by itself"* | every Phase B cluster report | [C29] |
| SD-82 | **The pilot boundary is a readiness criterion, not a date.** *"If we already know why an activity is structurally or causally wrong, having pilot coaches rediscover that gives us little useful information and spends their attention unnecessarily."* So internal diagnosis continues while the remaining failures are predominantly what the engine, contracts and controlled tests can establish. Pilot readiness becomes the question *"when the unresolved questions increasingly require observation of actual coach–learner–environment interaction rather than further deterministic diagnosis"* | prioritization | [C29] |
| SD-83 | **The establishment boundary.** *"Only support-capable, authoritative contributions may establish an element class. EXCLUSION, ASSUMED, ENGINE_ONLY, and OUTSIDE_BOUNDARY contributions do not establish existence."* An assumed contribution *"may constrain something whose existence is independently established, but it may not establish the element itself"*. Applied consistently across class formation; removal of the resulting 47 failed lines is **correction of falsely instantiated structure, not progress against knowledge gaps** | `derive.ts` `establishesExistence`; `formClasses` | [C30] |
| SD-84 | **Singleton collections establish singleton identity.** Where the authoritative schema defines a collection as cardinality exactly one, *"multiple independently support-capable existence contributions to that collection support the same singleton class unless authoritative knowledge explicitly establishes incompatibility."* **Not a general relaxation of SD-47** — for non-singleton collections derivation still manufactures no identity or equivalence. *"The distinction is that identity in the singleton case follows from the authoritative schema invariant itself rather than from interpretation of selectors, wording, or presumed equivalence."* Singletons are therefore read from the invariant **as data** — a citable standing decision stating `COUNT = 1` on a `COLLECTION` row — never from the row's prose | `register.ts` `singletonRows`; `V0` via SD-06 | [C30] |
| SD-85 | **An exclusion uses the negative-existence path.** *"An EXCLUSION does not participate in class formation. Where it forbids existence or cardinality beyond an allowed bound, evaluate it against the structure established by support-capable contributions through the existing negative-existence treatment."* No separate exclusion mechanism is created, because SD-75 already expresses the semantics | `forward.ts` | [C30] |
| SD-86 | **A bounded restatement of an exclusion's own authored cardinality.** It *"may move an explicitly authored numerical/cardinality bound from the existing prose into the typed field required by the negative-existence treatment, while preserving the original source and provenance"*. It may **not** infer a bound where none is stated, derive it from SD-06 instead of the item's own content, broaden or reinterpret the exclusion, or establish any new structural claim. **The relation is carried across as authored, not converted**: *"more than 1"* becomes `> 1`, not `>= 2`; *"2 or more"* becomes `>= 2`, not `> 1`. The prose value stays beside the typed form as its source | `corpus-restatements.json`; `forward.ts` | [C31] |
| SD-87 | **The goal-kick trigger, authored.** *"The state in which the ball leaves play over the defending team's goal line, having last been touched by an attacking player, without a goal being scored."* Mapped through the **existing** trigger and qualifier structure, with no schema change: `trigger = OUT_END_LINE` · `qualifier.endLine = DEFENDING_TEAM` · `qualifier.lastTouch = ATTACKING_TEAM`. *"Without a goal being scored"* is carried by `OUT_END_LINE` being a distinct member of the closed trigger list from `SCORE`, on a row *"keyed by trigger"* — one per transition, so the two are mutually exclusive. **It is not reduced to `OUT_END_LINE` alone**, which would also match a corner: `qualifier.lastTouch` is the discriminator, and a test asserts a corner does not reach the element. Clears all eight reference defects; no new row, attribute or register change | `restated:A01-02`; `corpus-restatements.json` | [C32] |
...
| `participation[] {group, state, persistence}` | NEUTRAL-05.a: *"active outfield participant for the whole activity: not rotated in and out, not a waiting or observing role"* had no row and refused its contract | SELECTION | The realized participant state of a performer group. **The concept is owned by the knowledge core (EM-0007 Participant State, EMD-0014); the Game Representation holds its realized value.** `state` takes EMP-0040's list and `persistence` EMP-0042's — bound, not re-authored (SD-71) |
...
| Field | Why | Owner | Claim |
|---|---|---|---|
| `rules.actionRestrictions[] {kind, effect, subject}` | GF2-22: *"Encourage progression without restricting passing options"* could be neither stated nor checked — no row held rules restricting play actions, and `V4` eligibility attaches only to the primary event | SELECTION | An authored rule that changes whether an otherwise available game action or target is legally eligible. `kind` is `receiver_eligibility`, `action_order` or `direction_class`; `effect` is `PROHIBITED` or `REQUIRED`. **Authored legal eligibility only** — nothing is inferred about tactical availability or perceptual affordance, and this is deliberately not a general rules ontology |
